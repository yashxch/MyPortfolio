...
import { Activity, ArrowLeft, ArrowUpRight, Cpu, Radio, Thermometer, Wifi } from "lucide-react";
import PageTransition from "@/components/layout/PageTransition";
import CaseStudyHeader from "@/components/work/CaseStudyHeader";
import MedicalMonitor from "@/components/work/MedicalMonitor";
import { PROJECTS } from "@/lib/data/projects";
import { useSound } from "@/components/ui/AudioToggle";

export default function PulseHaloCaseStudy() {
  const project = PROJECTS.find((p) => p.id === "pulsehalo") || PROJECTS[0];
      ...
      <article className="w-full px-6 sm:px-12 md:px-16 pt-36 pb-24 max-w-6xl mx-auto flex flex-col gap-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 pt-4">
          <div className="lg:col-span-4 flex flex-col gap-2 font-mono text-xs text-neutral-400">
            ...
            <span>NEONATAL BIOSENSOR SUITE</span>
            <span>ESP32 / AD8232 ECG / FIREBASE</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 pt-8 border-t border-white/10">
          <div className="lg:col-span-4 flex flex-col gap-2 font-mono text-xs text-neutral-400">
            ...
            <span>EDGE DSP & QRS DETECTION</span>
            <span>LOW-LATENCY STATE SYNC</span>
          </div>
        ...
        {/* 04: Interactive Medical Monitor Dashboard */}
          ...
          <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-2">
            <div>
              <span className="font-mono text-xs text-neutral-400 tracking-widest uppercase">
                // 04 INTERACTIVE INSTRUMENTATION
              </span>
              <h2 className="text-2xl font-bold text-white uppercase mt-1">
                Real-Time Neonatal Biosensor Monitor & Telemetry
              </h2>
            </div>
            <span className="font-mono text-xs text-red-400">
              [ LIVE ECG WAVEFORM & VITALS TELEMETRY ]
            </span>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 font-mono">
              {project.results.metrics.map((m) => (
                ...
                >
                  ...
                  <span className="text-2xl font-bold text-red-400">{m.value}</span>
        ...
        <div className="flex items-center justify-between pt-12 border-t border-white/10">
          ...
          <Link
            ...
            className="flex items-center gap-2 font-mono text-xs text-white hover:text-red-400 transition-colors"
          >
            <span>NEXT: 02 PREDICTIVE MAINTENANCE</span>
